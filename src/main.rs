...
fn main() {
	...
	start.push(Box::new(MovData::new(Register::EAX, Value::Int(4))));
	start.push(Box::new(MovData::new(Register::EBX, Value::Int(1))));
	start.push(Box::new(MovData::new(Register::ECX, Value::Pointer(".data".to_string()))));
	start.push(Box::new(MovData::new(Register::EDX, Value::Int(message.len() as u32))));
